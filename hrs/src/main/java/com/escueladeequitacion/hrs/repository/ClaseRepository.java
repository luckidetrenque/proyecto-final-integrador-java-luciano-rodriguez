...
public interface ClaseRepository extends JpaRepository<Clase, Long>, JpaSpecificationExecutor<Clase> {
  ...
  @Query("SELECT c FROM Clase c " +
      ...
      "LEFT JOIN FETCH c.alumno " +
      "LEFT JOIN FETCH c.personaPrueba " +
  ...
  public void deleteByDiaBetweenAndEstado(
  ...
  @Modifying
  @Query("DELETE FROM Clase c WHERE c.dia = :dia AND c.hora BETWEEN :horaInicio AND :horaFin AND c.estado = :estado")
  void deleteByDiaAndHoraBetweenAndEstado(
      @Param("dia") LocalDate dia,
  ...
  public boolean existsConflictoCaballo(
      ...
      @Param("dia") LocalDate dia,
      @Param("hora") LocalTime hora,
      @Param("caballoId") Long caballoId);
  ...
  // Verificar si un alumno ya tomó clase de prueba de una especialidad específica
  @Query("SELECT CASE WHEN COUNT(c) > 0 THEN true ELSE false END " +
      "FROM Clase c " +
      "WHERE c.alumno.id = :alumnoId " +
      "AND c.esPrueba = true " +
      "AND c.especialidad = :especialidad")
  boolean alumnoTieneClaseDePruebaEnEspecialidad(
      @Param("alumnoId") Long alumnoId,
      @Param("especialidad") Especialidad especialidad);

  // ── Finanzas: conteo de clases completadas por alumno ─────────────────
  @Query("""
          SELECT COUNT(c) FROM Clase c
          WHERE c.alumno.id = :alumnoId
            AND c.estado = :estado
      """)
  long contarPorAlumnoYEstado(
      @Param("alumnoId") Long alumnoId,
      @Param("estado") Estado estado);

  // ── Finanzas: conteo de clases completadas por instructor ─────────────
  @Query("""
          SELECT COUNT(c) FROM Clase c
          WHERE c.instructor.id = :instructorId
            AND c.estado = :estado
      """)
  long contarPorInstructorYEstado(
  ...
  @Query("""
      ...
      """)
  long contarPorInstructorEstadoYPeriodo(
      @Param("instructorId") Long instructorId,
      @Param("estado") Estado estado,
      @Param("inicio") LocalDate inicio,
      @Param("fin") LocalDate fin);
  ...
  @Query("""
          ...
          WHERE c.alumno.id = :alumnoId
            ...
            AND c.dia BETWEEN :inicio AND :fin
      """)
  long contarPorAlumnoEstadoYPeriodo(
      @Param("alumnoId") Long alumnoId,
      @Param("estado") Estado estado,
      @Param("inicio") LocalDate inicio,
      @Param("fin") LocalDate fin);
  ...
  public Page<Clase> findByEstado(Estado estado, Pageable pageable);

  public Page<Clase> findByEspecialidad(Especialidad especialidad, Pageable pageable);

  public Page<Clase> findByEstadoAndEspecialidad(Estado estado, Especialidad especialidad, Pageable pageable);
